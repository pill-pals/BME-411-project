function [lights lightsHist] = getOptimalLightLocationsWithoutGuess(width, height, n)
%     newLights = (rand(n,2) - 0.5);
%     newLights(:,1) = (newLights(:,1) * n/100) + width/2;
%     newLights(:,2) = (newLights(:,2) * n/100) + height/2;
    
    newLights = rand(n,2);
    newLights(:,1) = newLights(:,1) * width;
    newLights(:,2) = newLights(:,2) * height;
